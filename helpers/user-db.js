...
export const updateUserProfile = async (userId, updateData) => {
  ...
  try {
    const { name, surname, username, phone, profilePicture } = updateData;
    ...
    if (Object.keys(userFields).length > 0) {
      await User.update(userFields, { where: { Id: userId }, transaction });
    }

    //Actualizar tabla UserProfile
    const profileFields = {};
    if (phone) profileFields.Phone = phone;
    if (profilePicture) profileFields.ProfilePicture = profilePicture;

    if (Object.keys(profileFields).length > 0) {
      await UserProfile.update(profileFields, { where: { UserId: userId }, transaction });
    }
