...
export const sendVerificationEmail = async (email, name, verificationToken) => {
  ...
  try {
    ...
    const mailOptions = {
      ...
      to: email,
      subject: 'Verifica tu dirección de correo electrónico', // Aligned with .NET
      html: `
        ...
        <p>Si no puedes hacer clic en el enlace, copia y pega este enlace en tu navegador:</p>
        <p>${verificationUrl}</p>
...
export const sendPasswordResetEmail = async (email, name, resetToken) => {
  ...
  try {
    ...
    const mailOptions = {
      from: `${config.smtp.fromName} <${config.smtp.fromEmail}>`,
      to: email,
      subject: 'Restablecer contraseña', // Aligned with .NET
      html: `
        ...
        <p>Si no solicitaste este restablecimiento, por favor ignora este correo y tu contraseña no será modificada.</p>
      `,
    };
...
export const sendWelcomeEmail = async (email, name) => {
  ...
  try {
    const mailOptions = {
      from: `${config.smtp.fromName} <${config.smtp.fromEmail}>`,
      to: email,
      subject: '!Bienvenido a AuthDotnet!', // Aligned with .NET
      html: `
        <h2>!Bienvenido a AuthDotnet, ${name}!</h2>
...
export const sendPasswordChangedEmail = async (email, name) => {
  ...
  try {
    const mailOptions = {
      from: `${config.smtp.fromName} <${config.smtp.fromEmail}>`,
      to: email,
      subject: 'Contraseña actualizada correctamente', // More aligned with .NET style
      html: `
        <h2>Contraseña actualizada</h2>
        <p>Hola ${name},</p>
        <p>Tu contraseña ha sido actualizada exitosamente.</p>
        <p>Si no hiciste este cambio, por favor contacta a nuestro equipo de soporte inmediatamente.</p>
